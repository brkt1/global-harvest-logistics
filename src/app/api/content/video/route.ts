...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentType?.includes('multipart/form-data')) {
      ...
      writeFileSync(contentFilePath, JSON.stringify(videoData, null, 2))

      return NextResponse.json({ success: true, url: videoData.src })
    } else {
      // Handle JSON data
      const body = await request.json()
      const { key, src, title } = body
      ...
      writeFileSync(filePath, JSON.stringify(videoData, null, 2))
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!existsSync(filePath)) {
      return NextResponse.json({ src: '', title: '' })
    }

    const videoData = JSON.parse(readFileSync(filePath, 'utf8'))
    return NextResponse.json({ src: videoData.src, title: videoData.title })
  } catch (error) {
